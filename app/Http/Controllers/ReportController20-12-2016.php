...
class ReportController extends Controller
{
    ...
    public function getPublishPoint($std,$etd){ // calculate publish points
       /*$publishData = ProcessQueue::join('users','users.id', '=', 'process_queue.publish_by')
                            ->leftJoin('mst_status as pq_status','pq_status.id','=','process_queue.status_id')
                  ->select('total_lane','users.name')
                  ->whereRaw("process_queue.publish_end_date  BETWEEN '$std' AND '$etd'")
                              ->whereIn('pq_status.status_name',['sent to pricer','sent to customer','done', 'sent to audit'])
                  ->get();*/
        ...
        $publishArray = array();
          ...
          foreach($publishData as $publish){
           ...
           if($publish->total_lane >= 1 && $publish->total_lane <= 10){
            ...
            /* if($publish->total_lane >= 1 && $publish->total_lane <= 10){
             $publishArray['audit'][$publish->name] = $publishArray['audit'][$publish->name] + 1;
           }elseif($publish->total_lane >= 11 && $publish->total_lane <= 20){
             $publishArray['audit'][$publish->name] = $publishArray['audit'][$publish->name] + 2;
           }elseif($publish->total_lane >= 21 && $publish->total_lane <= 30){
             $publishArray['audit'][$publish->name] = $publishArray['audit'][$publish->name] + 3;
           }elseif($publish->total_lane >= 31 && $publish->total_lane <= 40){
             $publishArray['audit'][$publish->name] = $publishArray['audit'][$publish->name] + 4;
           }elseif($publish->total_lane >= 41 && $publish->total_lane <= 50){
             $publishArray['audit'][$publish->name] = $publishArray['audit'][$publish->name] + 5;
           }elseif($publish->total_lane > 50){
             $publishArray['audit'][$publish->name] = $publishArray['audit'][$publish->name] + 6;
           }*/

           }
          //dd($publishArray,$publishData->toArray());
       return $publishArray;
  ...
  public function getErrors($start_date,$end_date){ //fetch publish quotes for given date range
    ...
    public function productivityReport(Request $req)
    {
      ...
      if($req->from_date && $req->to_date){
        ...
        $end_date = date('Y-m-d 23:59:59'); 
         ...
         //dd($points);
         $publish = $points['publish']; // find publish points

         $audit = $points['audit'];//$this->getAuditPoint($start_date,$end_date); // find audit points

         $publishQuotes = $this->getPublishQuotes($start_date,$end_date); // find audit quotes

         $auditQuotes = $this->getPublishQuotes($start_date,$end_date); // find publish quotes
    ...
    foreach($userData as $user){
      ...
      $finalData['publish'][$user->name]    = isset($publish[$user->name]) ? $publish[$user->name] : '0';
        ...
        if($finalData['audit'][$user->name]!=0){
          $userCount = $userCount+1;
        }
    }
        if($userCount!=0){
          $finalData['mqc'] = $finalData['sum']/3;
        }else{
          $finalData['mqc'] = 0;
        }
