...
}elseif($auditing->action=='rfi'){
  $statusData = \App\Model\Status::where('status_type','apf')->where('status_name','=','pending out')->get();
}else{
  $statusData = \App\Model\Status::where('status_type','apf')->whereNotIn('status_name',['sent to customer','sent to inside sales'])->get();
}
?>
<div class="modal fade" id="apfCreateModal" role="dialog" data-backdrop="static" data-keyboard="false">
  <div class="modal-dialog modal-lg">
    <div class="modal-content">
      ...
      <form class="form-horizontal" role="form" method="POST" action="{{ route('auditing.update',$auditing->id) }}" id="apfForm1" autocomplete="off" >
          ...
          <div class="box-body">
            <div class="box box-default">
              ...
              <div class="box-body">
                      ...
                      <div class="col-md-3">
                         ...
                         <select name="user_name" id="name" class="form-control">
                          ...
                          @foreach(App\Model\User::all() as $user)
                            <option value="{{$user->id}}" {{(($user->id==(Session::get('user_id')))?'selected':'')}}>{{$user->name}}</option>
                          @endforeach
                        </select>
                        @else
                  ...
                  <div class="form-group">
                     <div id="resolved_section" class="{{((($auditing->status->status_name)=='pending out')?'':'hide')}}">
                         <label for="audit_rfi_comment" class="col-md-3 control-label">RFI Resolved Comment</label>
                          <div class="col-md-3 {{ $errors->has('audit_rfi_comment') ? ' has-error' : '' }}">
                          <textarea name="audit_rfi_comment" id="rfi_comment" class="form-control"></textarea>
                        ...
                        <div class="col-md-3">
                         <input type='text' class="form-control datetimepicker" name="rfi_etd" id="rfi_etd" value="" autocomplete="off"/>
                    ...
                    <div class="form-group">
                     ...
                     <div class="col-md-9">
                          <input type="checkbox" id="is_error" name="is_error" value="1" @if($auditing->publishing->is_error==1) checked @endif>
                ...
                <div id="rfi_section" class="{{((strtolower($auditing->request_type_name)=='rfi')?'':'hide')}}">
                   <div class="box box-default">
                      <div class="box-header with-border">
                      ...
                      <div class="box-body">
                        <div class="form-group">
                          ...
                          <div class="col-md-3">
                              <select name="audit_rfi_type_id" id="rfi_type_id" class="form-control">
                               <option value="">Select</option>
                                @foreach(\App\Model\RfiType::all() as $rfi)
                                  <option value="{{$rfi->id}}" {{(($rfi->id==$auditing->publishing->rfi_type_id)?'selected':'')}}>{{strtoupper($rfi->rfi_type_name)}}</option>
                                @endforeach
                              </select>
                          ...
                          <div class="col-md-3">
                               <textarea id="rfi_description" class="form-control" name="audit_rfi_description">{{ $auditing->publishing->rfi_description or old('audit_rfi_description') }}</textarea>
                       ...
                       <div class="form-group">
                           <label for="audit_isr_initiated" class="col-md-3 control-label">ISR Intiated</label>
                              <div class="col-md-3">
                              <input type="text" name="audit_isr_initiated" id="audit_isr_initiated" class="form-control" value="{{$auditing->publishing->isr_initiated or old('audit_isr_initiated')}}">
                ...
                @if(strtolower($auditing->request_type_name)!='cor' && $auditing->publishing->is_error!=1)
                <?php $class ="hide" ;?>
                @else
                <?php $class ="" ;?>
                @endif
                <div id="correction_section" class="{{$class}}">
                <div class="box box-default">
                  <div class="box-header with-border">
                  ...
                  <div class="box-body">
                          ...
                          <select name="audit_error_cat_id" id="audit_error_cat_id" class="form-control cor-drop">
                           <option value="">Select</option>
                            @foreach(\App\Model\ErrorCat::all() as $category)
                              <option value="{{$category->id}}" {{(($category->id==$auditing->publishing->error_cat_id)?'selected':'')}}>{{strtoupper($category->name)}}</option>
                            @endforeach
                          </select>
                          ...
                          <select name="audit_error_type_id" id="audit_error_type_id" class="form-control cor-drop">
                           <option value="">Select</option>
                            @foreach(\App\Model\ErrorType::all() as $type)
                              <option value="{{$type->id}}" {{(($type->id==$auditing->publishing->error_type_id)?'selected':'')}}>{{strtoupper($type->name)}}</option>
                            @endforeach
                          </select>
                    ...
                    <div class="form-group">
                      ...
                      <div class="col-md-3">
                          <textarea id="audit_error_description" class="form-control" name="audit_error_description">{{ $auditing->publishing->error_description or old('audit_error_description') }}</textarea>
                     ...
                     <label for="audit_root_cause" class="col-md-3 control-label">Root Cause Analysis <span style="color:#FF0000">*</span></label>
                      <div class="col-md-3">
                          <input id="root_cause" type="text" class="form-control" name="audit_root_cause" value="{{ $auditing->publishing->root_cause or old('root_cause') }}" >
                    ...
                    <div class="form-group">
                      ...
                      <div class="col-md-3">
                          <input id="audit_correction" type="text" class="form-control" name="audit_correction" value="{{ $auditing->publishing->correction or old('audit_correction') }}">
                      ...
                      <div class="col-md-3">
                          <input id="audit_corrective_action" type="text" class="form-control" name="audit_corrective_action" value="{{ $auditing->publishing->corrective_action or old('audit_corrective_action') }}">
                    ...
                    <div class="form-group">
                      ...
                      <div class="col-md-3">
                          <input id="audit_preventive_action" type="text" class="form-control" name="audit_preventive_action" value="{{ $auditing->publishing->preventive_action or old('audit_preventive_action') }}">
                      ...
                      <div class="col-md-3">
                          <input type='text' class="form-control datetimepicker" name="audit_proposed_comp_date" id="audit_proposed_comp_date" value="@if($auditing->publishing->proposed_comp_date!='0000-00-00 00:00:00' && !empty($auditing->publishing->proposed_comp_date)){{date('m/d/Y H:i:s',strtotime($auditing->publishing->proposed_comp_date))}} @endif"/>
                    ...
                    <div class="form-group">
                      ...
                      <div class="col-md-3">
                          <input type='text' class="form-control datetimepicker" name="audit_proposed_act_date" id="audit_proposed_act_date" value="@if($auditing->publishing->proposed_act_date!='0000-00-00 00:00:00' && !empty($auditing->publishing->proposed_act_date)){{date('m/d/Y H:i:s',strtotime($auditing->publishing->proposed_act_date))}} @endif" />
                      ...
                      <div class="col-md-3">
                          <select name="audit_error_done_by" id="audit_error_done_by" class="form-control cor-drop">
                              <option value="">Select</option>
                            @foreach(App\Model\User::all() as $user)
                              <option value="{{$user->id}}" {{(($user->id==$auditing->publishing->error_done_by)?'selected':'')}}>{{$user->name}}</option>
                            @endforeach
                          </select>
              ...
              <div class="form-group">
                  ...
                  <div class="col-md-3">
                      <textarea name="oot_remark" class="form-control" @if($auditing->oot==1) required @endif>{{$auditing->publishing->oot_remark or old('oot_remark')}}</textarea>
                  </div>
                  <label for="office" class="col-md-3 control-label">Comments</label>
                  <div class="col-md-3">
                      <textarea name="comments" class="form-control">{{old('comments')}}
                      </textarea>
