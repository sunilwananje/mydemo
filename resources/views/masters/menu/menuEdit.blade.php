    ...
    <div class="row">
        <div class="col-md-8 col-md-offset-2">
            <div class="box box-primary">
                ...
                @endif
                <form class="form-horizontal" role="form" method="POST" action="{{ route('menu.update',$recId->id) }}" autocomplete="off">
                    {{ csrf_field() }}
                    {{ method_field('PATCH') }}
                    <div class="box-body">
                        <div class="form-group{{ $errors->has('menu_name') ? ' has-error' : '' }}">
                            ...
                            <div class="col-md-6">
                                <input id="menu_name" type="text" class="form-control" name="menu_name" value="{{ $recId->menu_name }}" style="text-transform: capitalize;">
                            ...
                            <div class="col-md-6">
                                <select name="parent_id" class="form-control">
                                  ...
                                  @foreach(App\Model\Menu::all() as $menu)
                                    <option value="{{$menu->id}}" @if($recId->parent_id==$menu->id) selected @endif>{{$menu->menu_name}}</option>
                                  @endforeach  
                                </select>
                            ...
                            <div class="col-md-6">
                                <select name="permission_id" class="form-control">
                                  ...
                                  @foreach(App\Model\Permission::all() as $permission)
                                    <option value="{{$permission->id}}" @if($recId->permission_id==$permission->id) selected @endif>{{$permission->display_name}}</option>
                                  @endforeach  
                                </select>
